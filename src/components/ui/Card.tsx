...
export default function Card({ children, skip, handleSelect}: CardProps) {
  ...
  return (
    <div onClick={(e) => handleSelect(e, skip.id)} className={`rounded-lg shadow-md space-y-4 border ${skip.selected ? 'border-orange-600' : 'border-gray-200'} hover:border-orange-600 p-6 cursor-pointer relative`}>
      <span className="absolute px-3 py-1 rounded-3xl z-10 right-10 top-10 bg-orange-600 text-white text-sm">
        {skip.size}&nbsp;&nbsp;Yards
      </span>
      {!skip.allowed_on_road && <span className="font-sans text-xs absolute px-3 py-1 rounded-md z-10 left-8 sm:bottom-[47%] bottom-[50%] bg-black text-yellow-500 flex gap-2 items-center">
